...
namespace jsonpath {
    ...
    public:
        ...
        public:
            ...
            token(root_node_arg_t)
                : type_(token_type::root_node)
            {
            }
        ...
        class root_node final : public selector_base
        {
        public:
            root_node()
            {
        ...
        public:
            ...
            jsonpath_expression(static_context&& context,
    ...
    public:
        ...
        jsonpath_expression compile(const char_type* path, 
                                    std::size_t length,
                                    std::error_code& ec)
        {
                ...
                switch (state_stack_.back())
                {
                    ...
                    case path_state::lhs_expression: 
                    {
                        switch (*p_)
                        {
                            ...
                            case '$':
                                ++p_;
                                ++column_;
                                push_token(token(jsoncons::make_unique<root_node>()));
                                state_stack_.pop_back();
                                break;
                            case '@':
                                ++p_;
                            ...
                            default:
                                ...
                                {
                                    ec = jsonpath_errc::expected_identifier;
                                    return jsonpath_expression();
                                }
                                break;
                        };
                    ...
                    case path_state::literal: 
                        switch (*p_)
                        {
                            case '`':
                            {
                                ...
                                if (parse_ec)
                                {
                                    ec = jsonpath_errc::invalid_literal;
                                    return jsonpath_expression();
                                }
                    ...
                    case path_state::bracket_specifier:
                        switch(*p_)
                        {
                            ...
                            case '-':case '0':case '1':case '2':case '3':case '4':case '5':case '6':case '7':case '8':case '9':
                                ...
                                break;
                            default:
                                ec = jsonpath_errc::expected_index_expression;
                                return jsonpath_expression();
                        }
                        break;
                    ...
                    case path_state::expect_multi_select_list:
                        switch(*p_)
                        {
                            ...
                            case ':':
                            case '-':case '0':case '1':case '2':case '3':case '4':case '5':case '6':case '7':case '8':case '9':
                                ec = jsonpath_errc::expected_multi_select_list;
                                return jsonpath_expression();
                            case '*':
                                push_token(token(jsoncons::make_unique<list_projection>()));
                                state_stack_.back() = path_state::expect_right_bracket;
                    ...
                    case path_state::expect_right_brace:
                    {
                        switch(*p_)
                        {
                            ...
                            default:
                                ec = jsonpath_errc::expected_right_brace;
                                return jsonpath_expression();
                        }
                        break;
                    }
                    case path_state::expect_colon:
                    {
                        switch(*p_)
                        {
                            ...
                            case ':':
                                ...
                                break;
                            default:
                                ec = jsonpath_errc::expected_colon;
                                return jsonpath_expression();
                        }
                        break;
                    }
            ...
            return jsonpath_expression(std::move(context_), std::move(output_stack_));
        }
    ...
    template <class Json>
    using jsonpath_expression = typename jsoncons::jsonpath::detail::jsonpath_evaluator<Json,const Json&>::jsonpath_expression;

    template<class Json>
    Json json_query(const Json& doc, const typename Json::string_view_type& path)
    {
        jsoncons::jsonpath::detail::jsonpath_evaluator<Json,const Json&> evaluator;
        std::error_code ec;
    ...
    template<class Json>
    Json json_query(const Json& doc, const typename Json::string_view_type& path, std::error_code& ec)
    {
        jsoncons::jsonpath::detail::jsonpath_evaluator<Json,const Json&> evaluator;
        auto expr = evaluator.compile(path.data(), path.size(), ec);
        if (ec)
